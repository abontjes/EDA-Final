...
ASMax <- aggregate(cbind( Highest.Charting.Position, Number.of.Times.Charted, Artist.Followers)
                   ~ Artist, data = ASM, FUN = max)

ASMSum <- aggregate(cbind(Popularity, Danceability, Energy, Loudness, Speechiness, Acousticness,
                          Liveness, Tempo, Valence, Duration.ms,  Streams) ~ Artist, data = ASM, FUN = sum)

TopArtists <- merge(ASMSum, ASMax, by = 'Artist')
View(TopArtists)
